...
- **Movable center** — click anywhere on the canvas to place the convergence point (works at corners/edges too).
- **Draggable main-limb handles** — drag the dots around the center to aim each main limb.
...
- **~27 live parameters**, each with a 🔒 lock, plus **Randomize** (skips locked params).
...
- **Export video** — seed-sweep or growth animation → `.webm`, with optional per-frame randomization; live in-browser preview.
